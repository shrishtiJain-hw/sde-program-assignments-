...
SELECT
    ...
    IFNULL(pf.MINIMUM_STOCK, 0) AS REORDER_THRESHOLD,
    NOW() AS DATE_CHECKED
...
GROUP BY
    ...
    pf.MINIMUM_STOCK
HAVING
    SUM(ii.QUANTITY_ON_HAND_TOTAL) <= IFNULL(pf.MINIMUM_STOCK, 0)
    OR SUM(ii.AVAILABLE_TO_PROMISE_TOTAL) <= IFNULL(pf.MINIMUM_STOCK, 0)
ORDER BY
    ii.FACILITY_ID,
    ii.PRODUCT_ID;
```
